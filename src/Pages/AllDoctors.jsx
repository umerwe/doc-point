  ...
  return (
    <div className='text-gray-500'>
      <p className='font-medium py-5'>Browse through the doctors specialist.</p>
            <button
              onClick={() => { setIsOpen(!isOpen); setFilterOpen(true) }}
              className={`py-1 px-3 border rounded text-sm transition-all lg:hidden mb-3 text-black ${filterOpen ? 'bg-primary text-white' : ''}`}
            >
              Filters
            </button>
              ...
              <div className={`lg:flex lg:flex-col lg:static absolute top-10px bg-white w-[82%] sm:w-[400px] lg:gap-5 lg:w-[210px] ${isOpen ? 'block' : 'hidden'}`}>
                {specialityData.map((item, index) => (
                  <p
                    ...
                    className={`text-[13px] font-medium border border-gray-300 rounded py-1.5 text-left pl-4 pr-16 cursor-pointer duration-75 mb-3 lg:mb-0 ${speciality === item.speciality ? 'bg-[#EAEFFF] text-black' : ' text-gray-500'}`}
                  >
                    {item.speciality}
                  </p>
                ))}
